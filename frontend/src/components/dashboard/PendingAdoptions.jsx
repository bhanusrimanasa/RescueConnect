...
function PendingAdoptions() {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      alert(
        "Listing approved and published for adoption."
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">
          Adoption Listings Awaiting Admin Approval
        </h2>

        <p className="text-slate-400 text-xs sm:text-sm mt-0.5">
          These listings have passed volunteer verification and are waiting for your final decision.
        </p>
      </div>
      ...
      {requests.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center text-slate-400 text-xs sm:text-sm shadow-sm">
          No adoption listings awaiting approval.
        </div>
      ) : (
        <div className="space-y-6">
                  ...
                  <div className="flex-1 space-y-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                        <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Animal</p>
                    ...
                    <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-4 text-xs sm:text-sm text-slate-600 space-y-1.5">
                      <p className="flex items-center gap-1.5">
                        ...
                        <span>Submitted By: <strong className="text-slate-800">{request.submittedBy?.name || "Unknown"}</strong></span>
                      </p>

                      <p className="flex items-center gap-1.5">
                        <Phone size={14} className="text-slate-400 shrink-0" />
                        <span>Phone: <strong className="text-slate-800">{request.phone}</strong></span>
                      </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-slate-100">
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-100 sticky top-0 bg-white/95 backdrop-blur z-10">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center gap-2">
                  <PawPrint size={22} className="text-indigo-500" />
                  {selectedRequest.name}
                </h2>

                <p className="text-slate-400 text-xs sm:text-sm mt-0.5">
                  Full Adoption Listing Request
                </p>
              </div>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              {selectedRequest.images?.length > 0 ? (
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-6 text-center text-slate-400 text-xs sm:text-sm">
                  No images uploaded.
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs sm:text-sm">
                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  ...
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.name}</p>
                </div>
                ...
                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Breed</p>
                  <p className="font-semibold text-slate-800 mt-1 truncate">{selectedRequest.breed}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Age</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.age}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Gender</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.gender}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Size</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.size}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Location</p>
                  <p className="font-semibold text-slate-800 mt-1 truncate">{selectedRequest.location}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Phone</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.phone}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Vaccinated</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.vaccinated ? "Yes" : "No"}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Sterilized</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.sterilized ? "Yes" : "No"}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Special Needs</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.specialNeeds ? "Yes" : "No"}</p>
                </div>

                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Submitted By</p>
                  <p className="font-semibold text-slate-800 mt-1 truncate">{selectedRequest.submittedBy?.name || "Unknown"}</p>
                </div>
              </div>

              {selectedRequest.volunteerReviewedBy?.name && (
                <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3.5 text-xs sm:text-sm">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Volunteer Reviewer</p>
                  <p className="font-semibold text-slate-800 mt-1">{selectedRequest.volunteerReviewedBy.name}</p>
                </div>
              )}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-4 text-xs sm:text-sm text-slate-600 leading-relaxed whitespace-pre-line">
                    {selectedRequest.animalDescription}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-4 text-xs sm:text-sm text-slate-600 leading-relaxed whitespace-pre-line">
                    {selectedRequest.rescueStory}
                  </div>
                </div>
              </div>

              {/* Adoption Information */}
              {selectedRequest.adoptionReason && (
                <div className="space-y-2">
                  <h3 className="font-bold text-sm text-slate-900">
                    Adoption Information
                  </h3>
